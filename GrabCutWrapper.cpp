#include "GrabCutWrapper.hpp"

array<double, 2>^ GrabCutWrapper::ComputeForegroundProbability(String^ imagePath) {
...
cv::Mat GrabCutWrapper::MultiScaleCanny(const cv::Mat& image) {
...
std::tuple<cv::Mat, double, double, double> GrabCutWrapper::ComputeForegroundBackgroundProbability(
...
cv::Mat GrabCutWrapper::SimpleGrabCutForeground(const cv::Mat& image) {
    cv::Mat gray_image;
    cv::cvtColor(image, gray_image, cv::COLOR_BGR2GRAY);
